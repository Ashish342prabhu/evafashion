...
async function load() {
  try {
    const res = await fetch('/products/getproducts?category=beauty&category=jewelry')
...
function displayItems(data) {
  ...
  data.forEach(product => {
    innerHTML += `<div class="item-container">
      ...
      <div class="company-name">${product.titel}</div>
      <div class="itme-name">${product.brand }</div>
      <div class="price">
